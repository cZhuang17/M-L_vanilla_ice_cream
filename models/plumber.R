...
variables <- list(
  ...
  vendor = "The vendor identifier, categorical - Metro or Loblaws",
...
#* @param month 
#* @get /predict_price
predict_price <- function(old_price = 1.5, vendor = "Metro", month = 8) {
  ...
  beta_previous_price <- posterior_samples[, "old_price"]
  beta_ven <- posterior_samples[, "vendorLoblaws"]
  beta_mon <- posterior_samples[, "month"]
  ...
  predicted_values <- alpha +
    ...
    beta_vendor * ifelse(payload$vendor == "Loblaws", 1, 0) +
